...
function render(pkg, currentState) {
    ...
    const status = _([
        ...
        pkg.bump && !pkg.easyUpgrade ? [
            chalk.white.bold.bgGreen((pkg.bump === 'nonSemver' ? emoji(' :sunglasses: ') + ' new ver! '.toUpperCase() : emoji(' :sunglasses: ') + ' ' + pkg.bump.toUpperCase() + ' UP ')) + ' ' + uppercaseFirstLetter(pkg.bump) + ' 更新可用. ' + chalk.blue.underline(pkg.homepage || ''),
            indent + upgradeMessage
        ] : '',
        pkg.unused ? [
